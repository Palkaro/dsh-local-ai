...
- `test/` — vitest; REAL `Context`/`LlmRuntime`/`SystemPrompt`+`ToolRuntime`/`CommandRuntime`/local subprocess from the 0.1.0-rc.6 peers. Only the network edge (global `fetch`) is scripted.
...
- `typecheck` checks `src` + `test` against the published 0.1.0-rc.6 types; `typecheck:ci` clears `skipLibCheck` and adds `verbatimModuleSyntax` for the strict published-types pass. Both must stay green — the package ships against rc.6.
