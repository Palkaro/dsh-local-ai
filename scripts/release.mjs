#!/usr/bin/env node
/**
 * Release automation for dsh-local-ai.
 *
 ...
 *      tag (message: `dsh-local-ai v<version>`).
 *
 ...
 *
 * @module dsh-local-ai/scripts/release
 */
...
// The plugin version is stamped in src/version.ts (test/version.spec.ts trips
// when it drifts from package.json) — keep it in the same bump.
...
git('tag', '-a', `v${version}`, '-m', `dsh-local-ai v${version}`)
